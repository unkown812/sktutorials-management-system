...
const StudentAttendance: React.FC<StudentAttendanceProps> = ({ studentId }) => {
  ...
  const attendance = MOCK_ATTENDANCE.filter(
  ...
  const subjects = ['All', ...new Set(attendance.map(item => item.subject))];
  
  // Group attendance by subject for chart
  const subjectWiseAttendance = subjects
    ...
    .map(subject => {
      ...
      const subjectPresent = subjectAttendance.filter(item => item.status === 'Present').length;
      const percentage = subjectTotal > 0 ? (subjectPresent / subjectTotal) * 100 : 0;
            ...
            <tbody className="divide-y divide-gray-200">
              {attendance.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <span 
                      className={`badge ${item.status === 'Present' ? 'badge-green' : 'badge-red'}`}
                    >
                      {item.status}
                    </span>
                  </td>
